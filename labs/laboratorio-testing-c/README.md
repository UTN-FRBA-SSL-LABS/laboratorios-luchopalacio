...
> R: `carrito_init` inicializa el carrito con la cantidad de items en 0. Es importante llamarla antes de usar el carrito para asegurarnos que no contenga "basura" (valores indeterminados).
...
> R:No pasa, el valor esperado era 700 y obtuvo 350.
...
> R: El bug esta en la linea 24, donde se esta sumando solo el precio de cada producto, pero cada producto tambien tiene una cantidad que no tiene en cuenta

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Hay que multiplicar el precio por la cantidad
...
> R: El test falla porque carrito_agregar devuelve 1.
...
> R: Estaba usando "<=" lo cual es incorrecto porque se accedia a indices invalidos, entonces se deberia cambiar a "<".
